    ...
    private fun render(donationsList: ArrayList<DonationModel>) {
        ...
        if (donationsList.isNotEmpty()) {
            ...
            donationsList.forEach {
                markerColour = when {
                    it.email.equals(this.reportViewModel.liveFirebaseUser.value!!.email) ->
        ...
        private fun render(donationsList: ArrayList<DonationModel>) {
            if (donationsList.isNotEmpty()) {
                ...
                donationsList.forEach { donation ->
                    val markerColour = when {
                        donation.isFavorite -> BitmapDescriptorFactory.HUE_ORANGE
                        donation.email == reportViewModel.liveFirebaseUser.value?.email -> BitmapDescriptorFactory.HUE_AZURE + 5
                        else -> BitmapDescriptorFactory.HUE_RED
                    }

                    mapsViewModel.map.addMarker(
                        MarkerOptions().position(LatLng(donation.latitude, donation.longitude))
                            .title("${donation.paymentmethod} €${donation.amount}")
